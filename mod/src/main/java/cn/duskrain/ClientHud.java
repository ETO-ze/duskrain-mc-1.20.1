...
public final class ClientHud {
    ...
    public static void toggle(){visible=!enabled();try{Files.createDirectories(file().getParent());Files.writeString(file(),visible.toString());}catch(Exception e){DuskRain.LOG.warn("HUD preference",e);}ClientUI.notice(visible?"信息 HUD 已显示 · F8 隐藏":"信息 HUD 已隐藏 · F8 显示");}
    @SubscribeEvent public static void register(RegisterGuiOverlaysEvent event){event.registerAbove(VanillaGuiOverlay.PLAYER_LIST.id(),"information",(gui,g,partial,w,h)->{
        var mc=Minecraft.getInstance();if(mc.player==null||mc.screen!=null||mc.options.hideGui||mc.options.renderDebug||!enabled())return;
