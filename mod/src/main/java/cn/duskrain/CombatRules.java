...
public final class CombatRules {
    ...
    public double[][] damage={{2,.6,3},{2.2,.35,3.2},{1.5,2,.5}}, ranges={{16,5,8},{20,5,18},{6,5.5,0}};
    public double normalQiRange=8,normalQiDamage=.45, guardPve=.5,guardPvp=.25,frostCombo=.3;
    public double frostCastRange=16, lightningRadius=3.5;
    public int tuningRevision=0;
    ...
    public void validate(){
        ...
        for(double n:new double[]{base,stage,artifact,citySpeed,bodySpeed,bodyHealth,bodyResistance,jump,reach,swordMelee,intent,manaMultiplier,regenMultiplier,mageDiscount,pvp,normalQiRange,normalQiDamage,guardPve,guardPvp,frostCombo,frostCastRange,lightningRadius})if(!Double.isFinite(n)||n<0||n>100)throw new IllegalArgumentException("Nonfinite/negative tuning");
        if(ranges[0][1]>32||ranges[1][1]>32||ranges[2][1]>32||frostCastRange>32||lightningRadius>16||citySpeed>1||bodySpeed>1||bodyResistance>1||pvp>1||mageDiscount>1||guardPve>1||guardPvp>1||jump>1||reach>6||normalQiRange>32||normalQiCost<0||guardTicks<1||guardTicks>1200||frostTicks<20||frostTicks>1200||chargeTicks<1||chargeTicks>200||controlTicks<0||controlTicks>15)throw new IllegalArgumentException("Unsafe combat bounds");
    }
    static boolean upgrade(CombatRules next){
        if(next.tuningRevision>=1)return false;
        double[][] old={{14,4.5,8},{18,4.5,16},{6,5,0}}, fresh={{16,5,8},{20,5,18},{6,5.5,0}};
        for(int s=0;s<3;s++)for(int i=0;i<3;i++)if(next.ranges[s][i]==old[s][i])next.ranges[s][i]=fresh[s][i];
        if(next.normalQiRange==6)next.normalQiRange=8;
        next.tuningRevision=1;return true;
    }
    public static void load(){Path f=FMLPaths.CONFIGDIR.get().resolve("duskrain/combat.json");try{Files.createDirectories(f.getParent());if(!Files.exists(f))Files.writeString(f,Rules.JSON.toJson(current));var next=Rules.JSON.fromJson(Files.readString(f),CombatRules.class);next.validate();if(upgrade(next)){Path backup=f.resolveSibling("combat.before-fx-v1.json");if(!Files.exists(backup))Files.copy(f,backup);Files.writeString(f,Rules.JSON.toJson(next));}current=next;}catch(Exception e){DuskRain.LOG.error("Combat config rejected; keeping previous tuning",e);}}
}
