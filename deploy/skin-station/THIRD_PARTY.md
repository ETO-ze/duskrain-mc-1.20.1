# 组件与署名

| 组件 | 固定版本 / 来源 | 用途与许可 |
|---|---|---|
| Drasl | [官方仓库](https://github.com/unmojang/drasl)，4.0.1 | 皮肤站与 Yggdrasil 认证服务；GPL-3.0，许可副本见 `DRASL-LICENSE`。本目录的 layout/registration/header/player 模板由其模板修改。 |
| Blessing Skin / LittleSkin | [开源平台](https://github.com/bs-community/blessing-skin-server) / [LittleSkin 皮肤库](https://littleskin.cn/skinlib) / [接口说明](https://manual.littlesk.in/advanced/api) | 通过 LittleSkin 公共接口浏览与下载允许公开下载的皮肤和披风；开源平台为 GPL-3.0。本站不分发 LittleSkin 服务端，也不批量打包皮肤作品。在线接口查询于 2026-09-16，无固定内容版本。 |
| skinview3d | [官方仓库](https://github.com/bs-community/skinview3d)，3.4.1，Drasl 4.0.1 随附 | MIT，使用 Drasl 已随附的前端 bundle 实现 3D 预览；不额外加载第三方 CDN。 |
| Mojang 皮肤服务 | [Minecraft 皮肤设置](https://www.minecraft.net/msaprofile/mygames/editskin) | 查询公开的 Java 玩家档案及 textures.minecraft.net 皮肤与当前披风。Mojang 服务并非开源皮肤库；通过 Drasl 的皮肤转发功能提供跟随更新，未启用混合账号登录。 |
| authlib-injector | [官方仓库](https://github.com/yushijinhun/authlib-injector)，1.2.8 / build 56 | 外置认证 Java agent；遵循官方 GPL-3.0 及仓库所列附加条款。通过官方源安装。 |
| PCL2 | [官方仓库](https://github.com/Meloong-Git/PCL) | 使用用户本地启动器，仅配置新增版本；不打包启动器二进制。 |
| CustomSkinLoader | [官方仓库 v14.28](https://github.com/xfl03/MCCustomSkinLoader/tree/v14.28)、[ForgeV2 发行版](https://modrinth.com/mod/customskinloader/version/Rcbx2QhV) | GPL-3.0-only；客户端高清皮肤与披风解码。提供官方源安装脚本，未修改或重新打包其 JAR。版本、来源和 SHA-512 见 `assets/third-party/customskinloader-metadata.json`。 |
| Certbot | [官方仓库](https://github.com/certbot/certbot)，5.8.0 | 独立环境申请及续期 HTTPS 证书。 |
| DuskRain 主题 | 本项目原创 CSS、SVG 标志与首页内容 | 宣纸、青玉与印章主题；不使用第三方图片作为背景。 |

authlib-injector 官方下载：
`https://authlib-injector.yushi.moe/artifact/56/authlib-injector-1.2.8.jar`

SHA-256：`9c7f4343e6c82034958ffb48c14a2cb0c85928be7283103ce17da00c6d5a7b10`

Drasl 官方 amd64 镜像摘要：
`sha256:5646d68dc458431d6c2a4a757b77472d50e3881eeac51f68f76d88cbb7a13b37`

Minecraft 游戏本体、Forge 与客户端模组的来源及许可沿用项目原发行清单。游戏本体仅安装于用户本机，不加入源码库或公开服务器客户端 ZIP。

皮肤库保留每款作品的原站链接、名称和上传者。上传者不一定是原作者，具体归属和使用条件以原站与作者说明为准；开源程序许可不覆盖皮肤作品。仓库中的皮肤库页面及接口适配为本项目编写，公开接口请求不携带本站 Cookie、密码或登录令牌。
