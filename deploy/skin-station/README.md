...
“百衣藏”接入使用开源 Blessing Skin 的 LittleSkin 公共皮肤与披风库，支持分类、关键词、模型与排序筛选、分页、3D 旋转预览和应用到自己的角色。皮肤默认检索古风，披风默认浏览全部；披风不使用宽臂／细臂筛选。作品保留上传者及原站链接；平台的开源许可不覆盖每款作品的版权。

“正版外观识别”支持 Java 正版玩家名与 UUID，同时读取 `SKIN`、`CAPE` 与模型。可单独复制皮肤、单独复制披风或两项一起应用。没有 `CAPE` 时提示“当前未公开穿戴披风”，不提供虚假的披风结果。公开接口不列出账号所有披风，也不会授予正版账号权益。

整套跟随通过 Drasl `FallbackAPIServers` 完成：`ForwardSkins = true`、`EnableAuthentication = false`、`CacheTTLSeconds = 600`。Drasl 4.0.1 仅在没有本地皮肤和披风时转发正版外观；页面的“跟随整套外观”选择明确说明并清除两项本地材质。单独应用时，已有本地另一项不提交修改；来自跟随的另一项先以 PNG 保存，保留当前外观与皮肤模型，转为可混搭的本地材质。不修改角色 UUID，不启用混合账号登录。
